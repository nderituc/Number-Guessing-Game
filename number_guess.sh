...
DISPLAY() {
  ...
  if [[ $USER_ID ]]
  then
    GAMES_PLAYED=$($PSQL "SELECT COUNT(user_id) FROM guesses WHERE user_id = '$USER_ID'")

    #get best game (guess)
    BEST_GUESS=$($PSQL "SELECT MIN(guess) FROM guesses WHERE user_id = '$USER_ID'")
...
GAME() {
  ...
  echo -e "\nGuess the secret number between 1 and 1000:"
    ...
    then
      ...
      #insert into db
      INSERTED_TO_GAMES=$($PSQL "INSERT INTO guesses(user_id, guess) VALUES($USER_ID, $TRIES)")
      GUESSED=1
    #if greater
